...
export function isLlmEnabled(providerOverride?: LlmProvider) {
  ...
  const key = getApiKey(providerOverride ?? getProvider());
  if (!key) return false;
...
export function getDefaultModel(providerOverride?: LlmProvider) {
...
function getTimeoutMs(providerOverride?: LlmProvider) {
  const provider = providerOverride ?? getProvider();
